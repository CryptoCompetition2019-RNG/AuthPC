...
//		System.out.println("");
    ...
    private static int[][] expand(int[] B) {
        int W[] = new int[68];    //2176 bit
        int W1[] = new int[64];   //2048 bit
        for (int i = 0; i < B.length; i++) {
            W[i] = B[i];
        }
    ...
    private static byte[] bigEndianIntToByte(int num) {
        byte[] res = ConvertUtil.intToBytes(num);
    ...
    private static int bigEndianByteToInt(byte[] bytes) {
        ...
        return ConvertUtil.byteToInt(bytes);
    }
    ...
    public static byte[] padding(byte[] in, int bLen) {
        ...
        pos += padd.length;
        byte[] tmp = ConvertUtil.longToBytes(n);
    ...
    private static byte[] byteCycleLeft(byte[] in, int byteLen) {
        ...
        System.arraycopy(in, 0, tmp, in.length - byteLen, byteLen);//0-bytelen涓�捣鎸埌鏈�悗锛�
        return tmp;
    }
